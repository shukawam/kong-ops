...
```bash
# v1
https://api.toshogu.example.com/catalogue/v1/products

# v2
https://api.toshogu.example.com/catalogue/v2/products
```

### ヘッダーベースのマイナーバージョン

マイナーバージョンはヘッダーで指定できます（オプション）：

```bash
curl -H "Accept-Version: 1.2" \
  https://api.toshogu.example.com/catalogue/v1/products
```
...
1. **早期の対応**: 非推奨通知を受け取ったらすぐに移行計画を立てる
...
- **ダッシュボード**: Changelogセクション
- **API ドキュメント**: 各APIページ
- **GitHub**: リリースノート（一部API）
...
```bash
# RSSフィード
https://api.toshogu.example.com/changelog.rss

# Atomフィード
https://api.toshogu.example.com/changelog.atom
```
...
::accordion-group
  ...
  ::accordion-panel
  #header
  マイナーバージョンアップデートは必須ですか？
  #default
  いいえ。マイナーバージョンは後方互換性があるため、既存のコードは動作し続けます。ただし、新機能を利用したい場合はアップデートを検討してください。
  ::
::
